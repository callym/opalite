...
use crate::renderer::{ self, Buffer, BufferData, ModelKey, Model, RenderError, PushConstant, ShaderKey, Shader };
...
use hal;
use hal::{ command, device as d, format as f, image as i, memory as m, pass, pso };
...
impl MainPipe {
    ...
    pub fn draw(&mut self, command_buffer: &mut command::CommandBuffer<B, hal::Graphics>, frame_id: usize, model_locals: &[(&ModelKey, <Self as Pipe>::ModelsLocals)]) {
        let Self {
            pipeline_layout,
            render_pass,
            pipeline,
...

        {
            ...
            for (model_key, model_locals) in model_locals {
    ...
    pub fn new(
        ...
        let depth_view = {
            ...
            device.create_image_view(&depth_image, depth_format, f::Swizzle::NO, renderer::DEPTH_RANGE.clone())?
        };
